...
class Sk_model extends CI_Model
{
    ...
    public function hapus_multiple($id, $jmlData)
    {
        for ($i = 0; $i < $jmlData; $i++) {
            ...
            $files[] = $this->db->get_where('surat_keluar', ['id' => $id[$i]])->row_array(); // Dapatkan nama file setiap data.

            if ($files[$i]['file'] != null) {
                unlink(FCPATH . 'uploads/' . $files[$i]['file']); // hapus file dari folder uploads
            }

            $this->db->delete('surat_keluar', ['id' => $id[$i]]); // query hapus data surat keluar
            $this->db->delete('disposisi', ['sm_id' => $id[$i]]); // hapus disposisi surat keluar
        }
